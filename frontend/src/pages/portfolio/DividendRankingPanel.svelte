...
<div data-testid="dividend-ranking-panel">
  <div class="mb-3 flex items-center gap-2">
    ...
    <h3 class="text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.title")}</h3>
  </div>
      ...
      <table class="w-full text-sm" aria-label="Dividend Ranking">
        <thead class="border-b border-border-default bg-bg-secondary">
          <tr>
            ...
            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.ticker")}</th>
            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.indicator")}</th>
            ...
            <th class="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.payout")}</th>
            <th class="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.weight")}</th>
            <th class="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-text-muted">{t("dividendRanking.score")}</th>
          </tr>
        </thead>
          ...
          {#each items as item, i}
            ...
            <tr class="hover:bg-bg-tertiary {item.isHolding ? '' : 'opacity-70'}">
              ...
              <td class="px-3 py-2 font-medium">
                {item.ticker}
                {#if !item.isHolding}
                  <span class="ml-1 text-xs text-text-muted">{t("dividendRanking.watchlist")}</span>
                {/if}
              </td>
              <td class="px-3 py-2">
